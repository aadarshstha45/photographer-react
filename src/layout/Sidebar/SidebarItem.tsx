...
const SidebarItem: React.FC<ISidebarItem> = ({
  ...
  const borderColor = useColorModeValue("gray.700", "gray.50");
  ...
  return !!subItems && subItems.length > 0 ? (
    <>
      <HStack h={"full"} w={"full"} gap={3}>
        <Divider {...divider} opacity={pathname.includes(item.to) ? 1 : 0} />
        <Flex
          as={NavLink}
          bg={pathname.includes(item.to) || isOpen ? bg : "transparent"}
          color={pathname.includes(item.to) || isOpen ? color : ""}
          ...
          {subItems.map((subItem, index) => (
            ...
            >
              <Flex
                ...
                to={subItem.to}
                key={index}
                _activeLink={pathname === subItem.to ? active : {}}
                {...attributes}
              >
